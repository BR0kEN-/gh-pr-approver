import { log, skip } from './logger.mjs';
 ...
 * @param {string} org
 * @param {string} query
 * @param {Set<string>} acceptedAuthors
 *
 * @return {Promise<void>}
 */
export async function worker(octokit, approver, org, query, acceptedAuthors) {
  ...
  if (repos.length === 0) {
    ...
    if (pulls.length === 0) {
      ...
      for (const { url, title, number, user, draft, requested_reviewers, head } of pulls) {
        ...
        if (!acceptedAuthors.has(user.login)) {
